...
{
    global $documentroot;
    return $documentroot . '/views';
}

function includeviews(): string
{
    if (requestURI() == '/') {
        if (file_exists(documentview() . '/index.php')) {
            return documentview() . '/index.php';
        } else {
            return '';
        }
    } else {
        $txt = documentview() . '/' . requestURI();
        if (file_exists($txt)) {
            return $txt;
        } else {
            return '';
...
/* Exibe Alertas */
function windowAlert(string $title, string $message, string $type)
{
    echo sprintf("<script>window.miapp.mensagem('%s', '%s', '%s');</script>", $title, $message, $type);
}
